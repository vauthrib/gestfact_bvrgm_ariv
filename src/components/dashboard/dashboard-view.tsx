    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Bienvenue sur ARIV V2.81</p>
        </div>
          ...
          <TableBody>
            {tableRows.map((row, idx) => (
              <TableRow key={idx} className="hover:bg-blue-100/50">
                <TableCell className={`font-medium text-blue-800 border-r border-blue-200 ${row.isTTC ? 'text-xs' : ''}`}>{row.label}</TableCell>
                {[0, 1, 2, 3, 4, 5, 6].map((m) => {
