  ...
  return (
    <div className="p-6 space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-orange-700">Bons de Livraison</h1><p className="text-muted-foreground">Gérez vos BL</p></div>
        <div className="flex items-center gap-2">
          <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-sm font-mono font-bold">NBL01</span>
          ...
          {filtered.length === 0 ? <div className="text-center text-muted-foreground py-8">Aucun BL</div> : (
            <Table>
              ...
              <TableBody>{filtered.map((b) => (<TableRow key={b.id}>
                ...
                <TableCell><span className={`px-2 py-1 rounded text-xs ${b.statut === 'VALIDEE' ? 'bg-orange-100 text-orange-800' : 'bg-yellow-100 text-yellow-800'}`}>{b.statut === 'VALIDEE' ? 'Validé' : 'Brouillon'}</span></TableCell>
                <TableCell><div className="flex gap-1 flex-wrap">
                  {b.statut === 'BROUILLON' && <Button size="sm" variant="outline" className="text-orange-600" onClick={() => handleValidate(b.id)} title="Valider"><CheckCircle className="h-4 w-4" /></Button>}
                  {b.statut === 'VALIDEE' && <Button size="sm" variant="outline" className="text-orange-600" onClick={() => handleConvertToFacture(b)} title="Créer facture"><FileText className="h-4 w-4" /></Button>}
        ...
        <DialogContent className="max-w-[8000px] w-[calc(100vw-2rem)] max-h-[calc(100vh-2rem)] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-sm font-mono font-bold">NBL01-DLG</span>
            </div>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {editing ? (
                  ...
                  <div className="space-y-1">
                    <Input value={getProchainNumero()} disabled className="bg-gray-100 font-bold text-orange-700" />
            ...
            <DialogFooter><Button type="button" variant="outline" onClick={() => { setDialogOpen(false); resetForm(); }}>Annuler</Button><Button type="submit" className="bg-orange-600 hover:bg-orange-700">{editing ? 'Modifier' : 'Créer'}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
